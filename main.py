...
def build_rss(items, title):
    ...
    for item in items:
        ...
        img = f'<img src="{img_url}" /><br>' if img_url else ""
    ...
    async with async_playwright() as p:
        ...
        for link in links[:100]:
            try:
                ...
                title = normalize_name(await link.inner_text())

                parent = await link.evaluate_handle("el => el.closest('li')")

                member_el = await parent.query_selector("p")
                member = normalize_name(await member_el.inner_text()) if member_el else "不明"
